...
function acpwcheck(){
	...
	$.ajax({
		url : "/jigmBank_ver.2/account/accountcheck",
		type : "POST",
		...
		success : function(result){
			if(result == 1){
				...
				if(index2 >= 3){
					$.ajax({
						url : "/jigmBank_ver.2/account/accountactive",
						data : {"accountno" : arr},
						success : function(result){
							if(result == 1){
...
function transfer(){
	...
	$.ajax({
		url : "/jigmBank_ver.2/securitycard/usesccard",
		type : "POST",
		...
		success : function(result){
			if(result == 1){
				...
				if(index >= 3){
					$.ajax({
						url : "/jigmBank_ver.2/securitycard/sccardactive",
						data : {"accountno" : accountno},
						success : function(result){
							if(result == 1){
								alert("입력횟수제한-해당보안카드잠금(관리자문의)");
								location.href="/jigmBank_ver.2/main.jsp"
							}
						} // success end
					}); // ajax end
				} // if end
...
function transfer1(){
	...
	$.ajax({
		url : "/jigmBank_ver.2/transferready" ,
...
function transfer2(){
	...
	$.ajax({
		url : "/jigmBank_ver.2/transferlast" ,
		data : { "accnumr1" : achostno,"accnumr2":acguestno , "money":trfamount},
		type : "POST",
		success : function( result ){	/* 통신 성공시 받는 데이터 */
			if( result == 1 ){  
